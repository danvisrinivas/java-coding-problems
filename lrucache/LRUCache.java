...
public class LRUCache {
	...
	public void put(int id , String data) {
		...
		// We have to insert into cache + set it to be the head node
		if(this.actualSize<Constants.CAPACITY) {
			this.actualSize++;
			add(node);
		}else {
			//or the cache is full : we have to remove the item and insert the new one
			removeTail();
			add(node);
		}
	}
	...
	private void add(Node node) {
		...
		//we have to update the previous head node : point to the new node(which is the node)
	...
	// remove the last item (least frequently used)
	private void removeTail() {
		
		//get node from the heap
	...
	public Node get(int id) {
		...
		Node node = this.map.get(id);
		//move the head (frequently visited items must be close to the head)
		update(node);
		return node;
	}
